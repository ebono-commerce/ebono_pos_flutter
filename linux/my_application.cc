...
struct _MyApplication {
    ...
    char** dart_entrypoint_arguments;
    GtkWindow* main_window;  // Track the window
    gboolean user_activated; // Track if activation was user-initiated
};

G_DEFINE_TYPE(MyApplication, my_application, GTK_TYPE_APPLICATION)

/// Called when the window gains focus - REMOVED automatic fullscreen
static gboolean on_window_focus_in(GtkWidget* widget, GdkEventFocus* event, gpointer user_data) {
    // No longer automatically applying fullscreen on focus
    // This allows other applications to open on top normally
    return FALSE;
}
...
static void my_application_activate(GApplication* application) {
    ...
    // If window already exists, bring it to front and apply fullscreen
    // This handles dock icon clicks and explicit user activation
    if (self->main_window != NULL) {
        gtk_window_present(self->main_window);
        // Apply fullscreen when user explicitly activates from dock
        gtk_window_fullscreen(self->main_window);
        return;
    }
  ...
  if (GDK_IS_X11_SCREEN(screen)) {
    ...
    // Initial fullscreen on launch
    gtk_window_fullscreen(window);

    // REMOVED: gtk_window_set_keep_above - allows normal window stacking
    // This allows other applications to open on top normally

    // Keep the focus handler for potential future use, but it won't force fullscreen
    g_signal_connect(G_OBJECT(window), "focus-in-event", G_CALLBACK(on_window_focus_in), NULL);
